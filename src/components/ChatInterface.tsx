      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-8 scroll-smooth">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${
              msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-600 dark:text-gray-400'
            }`}>
            ...
            <div className={`max-w-[85%] sm:max-w-[70%] rounded-2xl p-5 shadow-sm ${
              msg.role === 'user'
                ...
                : 'bg-white dark:bg-gray-800 text-slate-800 dark:text-gray-100 rounded-tl-none border border-slate-100 dark:border-gray-700'
            }`}>
              {msg.role === 'assistant' ? (
        ...
        {loading && (
          <div className="flex gap-4">
            <div className="w-10 h-10 rounded-xl bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 flex items-center justify-center">
              <Bot size={20} className="text-blue-500" />
            </div>
            <div className="bg-white dark:bg-gray-800 border border-slate-100 dark:border-gray-700 rounded-2xl p-5 rounded-tl-none flex items-center gap-3 text-slate-400 italic">
      ...
      <div className="p-6 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800">
        <form onSubmit={sendMessage} className="relative max-w-4xl mx-auto">
          <input
            ...
            className="w-full p-4 pr-16 rounded-2xl border border-slate-200 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all bg-slate-50/50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
            disabled={loading}
          />
          <button
            type="submit"
        ...
        <div className="mt-3 flex justify-center">
          <p className="text-[10px] text-slate-400 dark:text-gray-600 font-bold uppercase tracking-widest">Expert Reasoning v2.0</p>
        </div>
      </div>
    </div>
  );
